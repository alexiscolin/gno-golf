...
/** The weather the ambience plays: the engine's, as much of it as there is. */
type Mood = Partial<WeatherNow>;

const KEY = "gnogolf.feel";
const prefs: Feel = (() => {
  try {
    return { sound: true, vibe: true, ...(JSON.parse(localStorage.getItem(KEY) || "{}") as Partial<Feel>) };
  } catch {
...
export function setFeel<K extends keyof Feel>(k: K, v: Feel[K]) {
  prefs[k] = v;
  if (k === "sound") ambience(v ? mood : {});
  try { localStorage.setItem(KEY, JSON.stringify(prefs)); } catch {}
}

export function buzz(p: VibratePattern) {
...
export const setSilent = (on: boolean) => ((silent = !!on), ambience(mood));
...
const audio = () => {
  if (!ctx) {
    const A = typeof window !== "undefined" && (window.AudioContext || (window as Window & { webkitAudioContext?: typeof AudioContext }).webkitAudioContext);
    if (!A) return null;
    ctx = new A();
  }
  if (ctx.state === "suspended" && present()) void resume(); // no gesture yet: the next one will do
  return ctx;
};
...
function tone(a: AudioContext, { type = "sine", f0, f1 = f0, at = 0, dur, gain = 0.2 }: { type?: OscillatorType; f0: number; f1?: number; at?: number; dur: number; gain?: number }) {
...
function noise(a: AudioContext, { at = 0, dur, f0, f1 = f0, q = 1, gain = 0.2 }: { at?: number; dur: number; f0: number; f1?: number; q?: number; gain?: number }) {
...
let mood: Mood = {}, bed: { src: AudioBufferSourceNode; g: GainNode; stop?: ReturnType<typeof setTimeout> } | null = null, gusts: ReturnType<typeof setTimeout> | undefined, hushed = false;
...
export function ambience(w: Mood = {}) {
  mood = w;
