...
export function makeWeather(scene: THREE.Scene, { onFlash = () => {}, camera = null }: { onFlash?: () => void; camera?: THREE.Camera | (() => THREE.Camera) | null } = {}) {
  ...
  return {
    ...
    set(zones: readonly WeatherZone[] | null, fc: Pick<Forecast, "wind"> | null = null): WeatherNow | null {
      ...
      const pz = (zones || []).filter((q): q is Zone => q.skin === "puddle" && !!q.min && !!q.max);
      puddles.forEach((p, k) => {
        const q = pz[k];
        p.visible = raining && !!q;
        if (!q) return;
    ...
    dispose() {
      ...
      for (const m of [bitMesh, ringMesh, puddleMesh, bankMesh, cloudMesh]) (m.geometry.dispose(), m.material.dispose(), m.dispose());
