...
const lit = (on) => (on ? new THREE.MeshBasicMaterial({ color: T.lampLit }) : flat(0x7f9aa6));

/** A tower of a house: a taller body, its cap, a clock face on the front. */
function clockTower(night) {
  const g = new THREE.Group();
 ...
 */
function townHouse(rand, night, only = null) {
  const g = new THREE.Group();
 ...
 */
function streetTree(rand, bare = false) {
  const g = new THREE.Group();
 ...
 */
function overhead(W, H, rand, night) {
  const g = new THREE.Group();
  const faders = [];
 ...
 */
function parkBed(rx, rz, rand) {
  const g = new THREE.Group();
  const shape = new THREE.Shape(), n = 14, pts = [];
  for (let i = 0; i < n; i++) {
 ...
 */
const HAZE = 0xf1d6c6;
function skyline(rand, X0, X1, Z0) {
  const g = new THREE.Group();
  ...
  const parts = [], hazeC = new THREE.Color(HAZE), col = new THREE.Color();
  const tinted = (geo, c, k, m) => {
...
function decor(s) {
  ...
  const onBoard = (x, z, r) => x > bx0 - 1.9 - r && x < bx1 + 1.9 + r && z > bz0 - 1.9 - r && z < bz1 + 1.9 + r;
  ...
  const moving = (o) => { for (let p = o.parent; p; p = p.parent) if (p.userData && p.userData.live) return true; return false; };
  const glows = [];
  g.traverse((o) => { if (o.isSprite && o.material === lanternGlow() && !moving(o)) glows.push(o); });
  if (glows.length) {
    const pos = new Float32Array(glows.length * 3);
    glows.forEach((o, i) => { const w = o.getWorldPosition(new THREE.Vector3()); pos.set([w.x, w.y, w.z], i * 3); o.parent.remove(o); });
...
const rough = {
  ...
  plant: (rand) => { const k = rand(); return k < 0.4 ? pot(rand) : k < 0.85 ? tflower(rand) : tbush(rand); },
};
...
function tramAcross(len, thick, night) {
  const g = new THREE.Group();
...
function stallAcross(len, thick, rand) {
  const g = new THREE.Group();
...
function awningAcross(len, thick) {
  const g = new THREE.Group();
...
function piece(kind, item, t, s) {
  ...
  if (kind === "zone") {
    ...
    if (skin === "stairs") {
      ...
      const g = new THREE.Group(), up = item.vec[0] !== 0 ? 0 : 1, n = 5;
      for (let i = 0; i < n; i++) {
        const k = (i + 0.5) / n;
    ...
    if (skin === "door" || skin === "well") {
      ...
      if (skin === "well") {
        g.add(onGround(well(r), cx, cz, t), onGround(well(r), item.vec[0], item.vec[1], t));
      } else {
        // the way in faces where the ball comes from; the way out faces on
        const into = Math.atan2(item.vec[1] - cz, item.vec[0] - cx);
        const a = onGround(doorway(false), cx, cz, t), b = onGround(doorway(true), item.vec[0], item.vec[1], t);
