...
const stripeTex = new Map();
function stripes(a, b, n = 6, vertical = true) {
  const key = `${a}-${b}-${n}-${vertical}`;
  if (stripeTex.has(key)) return stripeTex.get(key);
...
function boulder(p, y, color = C.stone, flatten = 0.8) {
...
function tram(c, L, th, ang, height) {
  const g = new THREE.Group();
  const y = height(c[0], c[1]);
...
function clockHand(c, L, th, ang, height) {
...
function stall(c, L, th, ang, height) {
  const g = new THREE.Group();
  const y = height(c[0], c[1]);
...
function awning(c, L, th, ang, height) {
...
export const MOUTHS = { cave, well, door, shipwreck };

/** Places a mouth object facing `dir` at (x, y, z). */
export function mouthAt(skin, x, y, z, R, dir) {
