...
function fireflies(rand: Rand, W: number, H: number) {
...
function signpost(label: string | number) {
  const g = new THREE.Group();
...
function hill(rand: Rand, far?: boolean) {
...
function house(cap: number = C.cap) {
  const g = new THREE.Group();
 ...
 */
function puddle(rx: number, rz: number, rand: Rand, color: number = C.pond) {
  const g = new THREE.Group();
  ...
  const flatShape = (pts: THREE.Vector2[], mat: THREE.Material, y: number) => {
  ...
  ud(g).outline = (grow) => outline(grow).map((p): MutVec2 => [p.x, -p.y]); // on the ground: x, z
  return g;
}

function pond(rand: Rand) {
  const g = puddle(2, 1.4, rand);
  // stones round it, sitting on the bank, never in the water
  for (const [x, z] of ud(g).outline!(0.35).filter((_, k) => k % 5 === 0)) {
    const st = stone(rand);
...
function fence(x0: number, x1: number, z: number) {
  ...
  const n = Math.round((x1 - x0) / 1.3), geos: THREE.BufferGeometry[] = [];
  for (let i = 0; i <= n; i++) {
...
function bunting(a: THREE.Vector3, b: THREE.Vector3) {
  ...
  const n = 13, sag = 0.9, pts: THREE.Vector3[] = [];
...
function smoke(at: THREE.Vector3) {
  const g = new THREE.Group();
  ud(g).live = true;
  ud(g).smokeAt = at.clone();
  state.smokes.push(g);
  return g;
}
...
export function smokeBatch(marks: readonly THREE.Object3D[]): { mesh: THREE.InstancedMesh; tick: Tick } | null {
  ...
  const tick = (t: number) => {
    ...
    marks.forEach((g, c) => {
      const at = ud(g).smokeAt!, on = shown(g);
...
function butterfly(rand: Rand, cx: number, cz: number, color: number) {
  const g = new THREE.Group();
...
function warp(x: number, y: number, z: number, r: number, color: number) {
  const g = new THREE.Group();
  ud(g).live = true;
...
function badge(color: number, dir: "down" | "up", x: number, y: number, z: number) {
  const c = document.createElement("canvas");
  c.width = c.height = 128;
  const ctx = c.getContext("2d")!;
  ctx.fillStyle = "#144134";
...
function mole(p: Pick<Post, "c" | "r">, height: Height) {
  const g = new THREE.Group();
  const [x, z] = p.c;
  const y = height(x, z);
