...
const THEME = { hole5: "giants", hole10: "giants", hole17: "giants", hole8: "clouds", hole14: "clouds", hole19: "clouds" };
const themeOf = (id) => {
...
function hills({ rand, X0, X1, reserve, free }) {
  const g = new THREE.Group();
  for (let i = 0; i < 4; i++) {
    const m = hill(rand, i % 2);
    const r = m.userData.r, x = X0 + 10 + ((X1 - X0 - 13) * i) / 3 + (rand() - 0.5) * 2, z = -ISLAND.back + 3.6;
    // its footprint is an oval 1.15r by 0.75r: two circles cover it
    const feet = [-1, 1].map((k) => [x + k * r * 0.55, r * 0.9]);
...
function ribbon(pts, half, side, v = (k) => k / (pts.length - 1)) {
  const pos = [], uv = [], idx = [];
  pts.forEach((p, k) => {
 ...
 */
function stream({ rand, X0, X1, reserve }) {
  const g = new THREE.Group();
  ...
  const across = (k) => { const q = curve.getTangent(k / N); return new THREE.Vector3(-q.z, 0, q.x).normalize(); };
  const width = (k) => 0.7 * Math.min(1, 0.35 + (k / N) * 9); // narrow at the spring, 1.4 wide soon after
  ...
  for (const [x, zz] of spring.userData.outline(0.3).filter((_, k) => k % 6 === 0)) {
 ...
 */
function farRange(rand, X0, X1, { tips = false } = {}) {
  const g = new THREE.Group();
...
function mountains({ rand, X0, X1, reserve, free }) {
  const g = new THREE.Group();
  ...
  for (const [t, h] of peaks) {
    ...
    if (t === 0.5) {
      ...
      g.add(fall);
      const { x: px, z: pz, r: pr } = fall.userData.pool;
      reserve(px, pz, pr);
    }
 ...
 */
function waterfall(base, dir, r, h, rand) {
  const g = new THREE.Group();
  const side = new THREE.Vector3(dir.z, 0, -dir.x);
  // a point on the ridge at height y, `lift` out from the rock
  const at = (y, lift = 0.25) => base.clone().addScaledVector(dir, r * (1 - y / h) + lift).setY(base.y + y);
  const top = h * 0.55;
  ...
  for (const [x, zz] of pool.userData.outline(0.35).filter((_, k) => k % 4 === 0)) {
    const st = stone(rand);
  ...
  const strand = (off, w0, w1, speed, start = 0, curl = 0.8) => {
  ...
  g.userData.pool = { x: centre.x, z: centre.z, r: 2.7 };
  return g;
}
 ...
 */
function giants({ rand, X0, X1, reserve, free, W, H }) {
  const g = new THREE.Group();
 ...
 */
const cloudy = (id) => themeOf(id) === "clouds";

/** Many puffs as one inked mesh (one draw, plus its outline). */
function puffs(list, inked = true) {
...
function cloudSea({ rand, W, H }) {
  ...
  A.userData.live = B.userData.live = true;
  g.add(A, B);
  animate((t) => {
...
const BACKDROPS = { hills, stream, mountains, giants };
...
function decor(s, bank = () => 0) {
  ...
  const FOOT = new Map([[bigFlower, 0.5], [tree, 1.1], [bush, 0.8], [stone, 0.5], [mushroom, 0.4], [flower, 0.25], [tuft, 0.2]]);
  const scatter = (n, x0, x1, z0, z1, make, k = 1) => {
  ...
  const low = (r) => { const t = rand(); return t < 0.45 ? bush(r) : t < 0.7 ? stone(r) : flower(r); };
  ...
  g.add(bd);
  if (bd.userData.fade) g.userData.fade = bd.userData.fade;
  ...
  if (["hills", "stream"].includes(themeOf(s.hole)) && hh % 2 === 0) {
    ...
    g.add(path);
    const extras = [[mailbox(), 0.4], [lantern(0, 0), 0.4]];
    homes.forEach(([x, z], k) => {
      if (k < extras.length) {
