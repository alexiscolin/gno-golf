 ...
 */
function instances(template, n) {
  compact(template);
  const g = new THREE.Group();
  g.userData.live = true;
  const meshes = template.children.filter((o) => o.isMesh).map((o) => {
...
function base(s, box) {
  ...
  const face = [], fi = [], steps = 60, drop = 16;
  ...
  const fcol = [], top = new THREE.Color(M.shadow), low = new THREE.Color(0xbcd3e6).lerp(new THREE.Color(M.rock), 0.35);
 ...
 */
function peaks(rand, X0, X1, Z0, Z1, drop, box) {
  const g = new THREE.Group();
  ...
  const geos = [], caps = []; // the snow caps apart: they lie on the rock faces
  ...
  const cone = (x, z, h, r, rot, k, y0) => {
    ...
    cap.rotateY(rot);
    const sc = snowC.clone().lerp(hazeC, k * 0.6), ccol = [];
  ...
  const skirt = (x, z, r, k, y0) => {
    ...
    const c = snowC.clone().lerp(new THREE.Color(0xc2d2e4), 0.45).lerp(hazeC, k), col = [];
  ...
  const massif = (x, z, h, k, y0 = GRASS + snowAt(W, H, x, z) - 0.5) => {
  ...
  const ridge = (z, k, y0, x0, x1, hgt) => {
    const pts = [], n = Math.ceil((x1 - x0) / 3);
...
function berms(s) {
  return { group: new THREE.Group(), height: (x, z) => snowAt(s.board.w, s.board.h, x, z) };
}

// ---------------------------------------------------------------- props

/** A pine weighed down with snow: three tiers, snow on each. */
function pine(rand, scale = 1) {
  const g = new THREE.Group();
 ...
 */
function chairLift(a, b, pylons, chairs, ground, night) {
  const g = new THREE.Group();
  ...
  const at = (u, s0, p = new THREE.Vector3()) => {
  ...
  for (const [p, lit] of [[a, true], [b, false]]) {
...
function skiers(rand, path, ground, n) {
 ...
 */
function bobsleigh(X0, X1, H, bank, reserve) {
  const g = new THREE.Group();
...
function frozenPond(x, y, z, rand) {
  const g = new THREE.Group();
...
function tracks(rand, X0, X1, z0, z1, W, gaps = []) {
  const inGap = (x) => gaps.some(([a, b]) => x > a - 0.4 && x < b + 0.4);
  const g = new THREE.Group();
  ...
  for (let t = 0; t < 2; t++) {
    ...
    const c = new THREE.CatmullRomCurve3(pts), runs = [[]];
    for (let k = 0; k <= 120; k++) {
...
function decor(s, bank = () => 0) {
  ...
  const pondAt = [X0 + (X1 - X0) * 0.2, (front0 + front1) / 2 + 0.5];
...
const green = [0xeaf4ff, 0xd8e7f8]; // a little blue: the warm light turns plain white beige
const edgeInk = false;
...
function extras(ex, s, t) {
  ...
  const W = s.board.w, H = s.board.h;
  const ground = (x, z) => (x >= 0 && x <= W && z >= 0 && z <= H ? height(x, z) : GRASS + snowAt(W, H, x, z));
  const clear = (x, z, r) => toWall(x, z) > r && (s.posts || []).every((p) => Math.hypot(p.c[0] - x, p.c[1] - z) > p.r + r);
...
function heap(b, height, ground, clear, start, down = 0) {
  ...
  const len = 5, N = 16, M = 8, [ox, oz] = b.out, sx = -oz, sz = ox, pos = [], col = [], idx = [], ok = [];
...
function warning(w, ground, start) {
  const g = new THREE.Group();
 ...
 */
function overlay(z, t, col, bump = () => 0, mat, lane = true, mask = null) {
...
const snowRise = (lo, hi, top) => (x, z, h) => new THREE.Color(lo).lerp(new THREE.Color(hi), Math.min(1, h / top));

/** A pine as a post: its trunk is the post's circle, its boughs high above. */
function pinePost(r, rand) {
...
const crevasses = (s) => (s.zones || []).filter((z) => z.skin === "crevasse" && z.min[1] <= 0.01 && z.max[1] >= s.board.h - 0.01).map((z) => [z.min[0], z.max[0]]);

/** A chasm: ice walls going blue to black, jagged snow lips, mist deep down. */
function crevasse(z, t, ends = false) {
  const g = new THREE.Group();
  const [x0, z0] = z.min, [x1, z1] = z.max, D = -7, rand = seeded("crev" + z.min);
  ...
  for (const [at, sgn] of walls) {
    ...
    const n = Math.ceil(L / 0.6), rows = 6, pos = [], col = [], idx = [];
    ...
    const n2 = Math.ceil(L / 0.25), P = 7, cpos = [], cidx = [];
    for (let k = 0; k <= n2; k++) {
      const u = (k / n2 - 0.5) * L;
...
function cliff(z, t, s) {
  ...
  const wall = (ax, az, bx, bz) => {
    const L = Math.hypot(bx - ax, bz - az), n = Math.max(2, Math.ceil(L / 0.6)), rows = 6, pos = [], col = [], idx = [];
    for (let j = 0; j <= rows; j++)
      for (let i = 0; i <= n; i++) {
  ...
  if (!z.poly)
    for (const [ez, sgn] of [[z0, -1], [z1, 1]]) {
      ...
      const n2 = Math.ceil(zw / 0.25), P = 7, cpos = [], cidx = [];
      for (let k = 0; k <= n2; k++) {
...
function snowCannon(z, t, s) {
  ...
  const safe = (x, zz) => (!t.onGreen || t.onGreen(x, zz)) && !drops.some((q) => inZone(q, x, zz));
  ...
  const chevGeo = new THREE.ShapeGeometry(chev), marks = [];
  ...
  for (let k = 0; k < 14; k++) {
    ...
    for (let v = 0; v < 1; v += 0.12) {
      const p0 = [x + dx * v * reach, zz + dz * v * reach], p1 = [p0[0] + dx * 0.5, p0[1] + dz * 0.5];
...
function piece(kind, item, t, s) {
  ...
  const [x0, z0] = item.min, [x1, z1] = item.max, cx = (x0 + x1) / 2, cz = (z0 + z1) / 2;
  if (skin === "ice") {
    ...
    const scratch = [], crack = [];
    const add = (arr, p, q, lift) => { for (const [a2, b2] of [p, q]) arr.push(new THREE.Vector3(a2, t.height(a2, b2) + lift, b2)); };
  ...
  if (item.kind === "slope" && ["kicker", "terrace", "downhill", "slope", "saddle crest", "bank"].includes(skin)) {
    ...
    const l = Math.hypot(item.vec[0], item.vec[1]) || 1, ux = -item.vec[0] / l, uz = -item.vec[1] / l;
    ...
    if (skin === "kicker" || skin === "terrace") {
      ...
      const green = (x, zz) => !t.onGreen || t.onGreen(x, zz);
      let lo = Infinity, hi = -Infinity;
      for (let k = 0; k <= 80; k++) {
  ...
  if (skin === "crevasse" && !(item.min[1] <= 0.01 && item.max[1] >= s.board.h - 0.01)) return crevasse(item, t, true);
  if (item.kind === "tunnel" && (skin === "ice cave" || skin === "door")) {
    const g = new THREE.Group();
    const face = (x, zz) => Math.atan2(x - cx, zz - cz); // facing where the ball comes from
    const tee = s.start;
    if (skin === "ice cave") {
      const r = Math.min(x1 - x0, z1 - z0) / 2;
      const inn = onGround(iceCave(r), cx, cz, t);
      inn.rotation.y = face(tee[0], tee[1]);
      const out = onGround(iceCave(1.1), item.vec[0], item.vec[1], t);
      out.rotation.y = inn.rotation.y + Math.PI;
      g.add(inn, out);
    } else {
      const inn = onGround(doorway(false), cx, cz, t);
      inn.rotation.y = face(tee[0], tee[1]);
      const out = onGround(doorway(true), item.vec[0], item.vec[1], t);
      out.rotation.y = inn.rotation.y + Math.PI;
      g.add(inn, out);
    }
    return g;
