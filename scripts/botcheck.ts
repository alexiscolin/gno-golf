...
//
//   nice -n 20 node --experimental-strip-types scripts/botcheck.ts [--rpc URL] [--top 10] [--delay 150]
...
// (Node 22.6+ runs it as TypeScript; from 23.6 the flag is on by default.)
...
function opt(name: string, def: boolean): boolean;
function opt(name: string, def: string | number): string;
function opt(name: string, def: string | number | boolean): string | boolean {
  const i = argv.indexOf(`--${name}`);
  return i < 0 ? (typeof def === "number" ? String(def) : def) : typeof def === "boolean" ? true : (argv[i + 1] ?? "");
}
...
interface Best { par: number; best: number; robust?: number; period?: number; plan?: string[]; raw?: string[] }
const BESTS = JSON.parse(fs.readFileSync(here("./hole-bests.json"), "utf8")) as Record<string, Best>;

// --- rate-limited chain -------------------------------------------------------

const chain = makeChain({ rpc: RPC });
let lastCall = 0, calls = 0, sims = 0;
async function rpc<T>(fn: (c: Chain) => Promise<T>): Promise<T> {
...
/** A finished round on record. */
interface Rec { hole: string; shots: string[]; strokes: number; period: number }

const parseShot = (s: string): Shot => {
...
function valueSignal(shots: readonly Shot[]) {
  if (!shots.length) return 0;
  const per: number[] = shots.map(({ a, p }) => (near(a, 1) && near(p, 0.25) ? 1 : near(a, 0.01) && near(p, 0.01) ? 0 : 0.5));
...
const NUDGES: [number, number, number][] = [[0.2, 0, 0], [-0.2, 0, 0], [0, 0.05, 0], [0, -0.05, 0]];
async function fragility(r: Rec) {
  const shots = r.shots.map(parseShot), last = shots[shots.length - 1], prefix = shots.slice(0, -1).map(fmt);
  const nudges: [number, number, number][] = [...NUDGES, ...(last.t > 0 ? [[0, 0, 1], [0, 0, -1]] as [number, number, number][] : [])]; // at most 6
  let tried = 0, held = 0;
...
export async function scorePlayer({ bests, rounds }: { bests: Record<string, number>; rounds: Rec[] }) {
  ...
  const score = (Object.keys(WEIGHTS) as Signal[]).reduce((x, k) => x + WEIGHTS[k] * s[k], 0);
...
async function run() {
  const holes = (await rpc((c) => c.holes())).flatMap((h) => (h.official && !h.next && h.slot ? [h.slot] : []));
...
function report({ holes, players }: Awaited<ReturnType<typeof run>>) {
...
async function selftest() {
  ...
  const bot = { bests: {} as Record<string, number>, rounds: [] as Rec[] }, human = { bests: {} as Record<string, number>, rounds: [] as Rec[] };
  ...
  for (const [hole, b] of picked) {
    const plan = (b.raw || b.plan || []).map(parseShot), period = b.period ?? 0;
    const res = await simulate(hole, plan.map(fmt), period);
    if (!res.holed) { console.log(`skip ${hole}: its plan no longer holes on this chain`); continue; }
    bot.bests[hole] = res.strokes;
    bot.rounds.push({ hole, shots: plan.map(fmt), strokes: res.strokes, period });
    ...
    for (let k = 0; k < 5; k++) {
      ...
      const r = await simulate(hole, noisy.map(fmt), period);
      if (r.holed && r.strokes <= res.strokes) {
        human.bests[hole] = r.strokes;
        human.rounds.push({ hole, shots: noisy.map(fmt), strokes: r.strokes, period });
        break;
      }
    }
  }
